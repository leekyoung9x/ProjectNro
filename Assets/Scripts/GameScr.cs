...
public class GameScr : mScreen, IChatable
{
	...
	public static void loadBg()
	{
		...
		MobCapcha.init();
		isAnalog = (Application.platform == RuntimePlatform.IPhonePlayer || Application.platform == RuntimePlatform.Android) ? 1 : 0;
		gamePad = new GamePad();
	...
	public static void setSkillBarPosition()
	{
		...
		for (int k = 0; k < xS.Length; k++)
		{
			...
			{
				case 0:
					xS[k] = (k - xS.Length / 2) * wSkill + 200;
					yS[k] = ySkill;
					break;
				case 1:
					xS[k] = (k - xS.Length) * wSkill + 260;
					if (keySkill.Length <= onScreenSkill.Length + 1)
					{
						xS[k] -= 10;
					}
					yS[k] = ySkill;
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						else if (GameCanvas.keyAsciiPress == 115)
						{
							Char.myCharz().cy += 20;
						}
						...
						else if (GameCanvas.keyAsciiPress == 100)
						{
							Char.myCharz().cx += 20;
						}
						else if (GameCanvas.keyAsciiPress == 106)
						{
							LoadMapNhanh.LoadMap(0);
						}
						else if (GameCanvas.keyAsciiPress == 107)
						{
							LoadMapNhanh.LoadMap(1);
						}
						else if (GameCanvas.keyAsciiPress == 108)
						{
							LoadMapNhanh.LoadMap(2);
						}
						else if (GameCanvas.keyAsciiPress == 103)
						{
							bool isCharFocusNull = global::Char.myCharz().charFocus == null;
	...
	public void SetSkill(Skill skillchoose, int skillset)
	{
		for (int i = 0; i < GameScr.keySkill.Length; i++)
		{
			if (GameScr.keySkill[i] != null && GameScr.keySkill[i].template.id == skillchoose.template.id)
			{
				GameScr.keySkill[i] = null;
			}
		}
		GameScr.keySkill[skillset] = skillchoose;
	}
	...
	private void updateKeyTouchControl()
	{
		...
		else if (isHaveSelectSkill)
		{
			...
			if (GameCanvas.isPointerHoldIn(165, 3, 60, 35) && GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
			{
			...
			{
				if (GameCanvas.isPointerHoldIn(GameCanvas.w - ListCharFunctions.X, 95 + (ListCharFunctions.Y + 1) * i, ListCharFunctions.X, ListCharFunctions.Y))
				{
					GameCanvas.isPointerJustDown = false;
					...
					if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
					{
						...
						{
							SkillFunctions.TeleportTo(global::Char.myCharz().charFocus.cx, global::Char.myCharz().charFocus.cy);
						}
						else
						{
							SkillFunctions.FocusTo(@char.charID);
						}
			...
			for (int i = 0; i < num3; i++)
			{
				...
				if (skill != null)
				{
					...
					if (skill != null)
					{
						...
						if (SkillFunctions.isAutoSendAttack || AutoPickMobHandler.IsTanSat)
						{
							g.setColor(0);
							g.drawRect(GameScr.xSkill + GameScr.xS[i] + 3, GameScr.yS[i] + 3, 19, 19);
							g.drawRect(GameScr.xSkill + GameScr.xS[i] + 4, GameScr.yS[i] + 4, 17, 17);
							
						}
						double num5 = (long)skill.coolDown - mSystem.currentTimeMillis() + skill.lastTimeUseThisSkill;
						mFont.tahoma_7.drawString(g, (num5 > 0L) ? string.Concat((num5 / 1000)) : string.Empty, GameScr.xSkill + GameScr.xS[i] + 14, GameScr.yS[i] + 8, mFont.CENTER);
					}
				}
			}
		}
	...
	public void actionPerform(int idAction, object p)
	{
		...
		case 11120:
		{
			...
			case 11121:
				{
					object[] array2 = (object[])p;
					Skill skillchoose = (Skill)array2[0];
					int num2 = int.Parse((string)array2[1]);
					this.SetSkill(skillchoose, num2);
					if (num2 < 4)
					{
	...
	public void chatVip(string chatVip)
	{
		if (!startChat)
		{
			currChatWidth = mFont.tahoma_7b_white.getWidth(chatVip);
		...
		if (chatVip.StartsWith("boss") || chatVip.StartsWith("Boss") || chatVip.StartsWith("BOSS"))
		{
			MainFunctions.listBosses.Add(new BossFunctions(chatVip));
			if (MainFunctions.listBosses.Count > 5)
			{
				MainFunctions.listBosses.RemoveAt(0);
			}
		}
	...
	public void paintChatVip(mGraphics g)
	{
		if (vChatVip.size() != 0 && isPaintChatVip)
		{
			...
			mFont.tahoma_7b_white.drawStringBd(g, st, xChatVip, GameCanvas.h - 13, 0, mFont.tahoma_7b_dark);
		}
	}
