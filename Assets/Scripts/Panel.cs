...
public class Panel : IActionListener, IChatable
{
		...
		for (int i = 0; i < currentListLength; i++)
		{
			...
			if (item.itemOption != null)
			{
				for (int k = 0; k < item.itemOption.Length; k++)
				{
					...
					{
						mFont.tahoma_7b_white.drawStringBd(g, item.itemOption[k].param.ToString() + "s", num + 120, num2, 0, mFont.tahoma_7b_dark);
					}
		...
		if (cmtoX >= wScroll - 10 && cmx >= wScroll - 10 && position == 0)
		{
			...
			if (isChangeZone)
			{
				...
				if (Char.myCharz().cHP > 0 && Char.myCharz().statusMe != 14)
				{
					...
					{
						MapService.SelectMapTrans(this.selected);
					}
				}
			}
	...
	public void paintTop(mGraphics g)
	{
		...
		for (int i = num2; i < num; i++)
		{
			...
			if (Char.myCharz().isTickBlue)
            {
               Char.myCharz().paintTickBlue(g, num6 + 5 + mFont.tahoma_7b_green.getWidth(topInfo.name), num7);
			...
			if (item != null)
			{
				...
				if (item.itemOption != null)
				{
					for (int k = 0; k < item.itemOption.Length; k++)
					{
						...
						{
							mFont.tahoma_7b_white.drawStringBd(g, item.itemOption[k].param.ToString() + "s", num3 + 120, num4, 0, mFont.tahoma_7b_dark);
						}
						if (item.itemOption[k].optionTemplate.id == 41)
						{
							if (item.itemOption[k].param == 1)
			...
			for (int i = 0; i < currentListLength; i++)
			{
				...
				if (item.itemOption != null)
				{
					for (int l = 0; l < item.itemOption.Length; l++)
					{
						...
						{
							mFont.tahoma_7b_white.drawStringBd(g, item.itemOption[l].param.ToString() + "s", num2 + 120, num3, 0, mFont.tahoma_7b_dark);
						}
	...
	private void paintLogChat(mGraphics g)
	{
		...
		for (int i = 0; i < currentListLength; i++)
		{
			...
			if (Char.myCharz().isTickBlue)
            {
               Char.myCharz().paintTickBlue(g, num4 + 10 + tahoma_7b_dark.getWidth(infoItem.charInfo.cName),
                    num5  + 11);
	...
	private void paintEnemy(mGraphics g)
	{
		...
		for (int i = 0; i < currentListLength; i++)
		{
			...
			if(Char.myCharz().isTickBlue){
						Char.myCharz().paintTickBlue(g, num4 + 5 + ((infoItem.isOnline) ? mFont.tahoma_7b_green.getWidth(infoItem.charInfo.cName) : mFont.tahoma_7_grey.getWidth(infoItem.charInfo.cName)),
                    num5);
	...
	private void paintClans(mGraphics g)
	{
		...
		for (int j = 0; j < currentListLength; j++)
		{
			...
			if (isViewMember)
			{
				...
				if(Char.myCharz().isTickBlue){
						Char.myCharz().paintTickBlue(g,  num6 + 9 + mFont2.getWidth(member.name), num7 + 11);
		}
		...
		for (int j = 0; j < vItemCombine.size() + 1; j++)
		{
			...
			if (item.itemOption != null)
			{
				for (int l = 0; l < item.itemOption.Length; l++)
				{
					...
					{
						mFont.tahoma_7b_white.drawStringBd(g, item.itemOption[l].param.ToString() + "s", num + 120, num2, 0, mFont.tahoma_7b_dark);
					}
					if (item.itemOption[l].optionTemplate.id == 41)
					{
						if (item.itemOption[l].param == 1)
			...
			for (int j = 1; j < currentListLength; j++)
			{
				...
				if (item.itemOption != null)
				{
					for (int l = 0; l < item.itemOption.Length; l++)
					{
						...
						{
							mFont.tahoma_7b_white.drawStringBd(g, item.itemOption[l].param.ToString() + "s", num4 + 120, num5, 0, mFont.tahoma_7b_dark);
						}
						if (item.itemOption[l].optionTemplate.id == 41)
						{
							if (item.itemOption[l].param == 1)
				...
				mFont2.drawString(g, "[" + item.template.id + "] " + item.template.name + text, num4 + 5, num5 + 1, 0);
				string text2 = string.Empty;
				if (item.itemOption != null)
				{
					if (item.itemOption.Length > 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
	...
	private void paintTab(mGraphics g)
	{
		...
		{
			for (int i = 0; i < MenuHandler.MenuOption.Length; i++)
			{
				...
				if (!MenuHandler.MenuOption[i][1].Equals(string.Empty))
				{
					mFont.drawString(g, MenuHandler.MenuOption[i][0], this.startTabPos + i * this.TAB_W + this.TAB_W / 2, 53, mFont.CENTER);
					mFont.drawString(g, MenuHandler.MenuOption[i][1], this.startTabPos + i * this.TAB_W + this.TAB_W / 2, 64, mFont.CENTER);
				}
				else
				{
	...
	private void paintToolInfo(mGraphics g)
	{
		...
		if(Char.myCharz().isTickBlue ){
						Char.myCharz().paintTickBlue(g,64 + mFont.tahoma_7_yellow.getWidth(mResources.character + ": " + Char.myCharz().cName), 27);
	...
	private void paintCharInfo(mGraphics g, Char c)
	{
			...
			if (true)
			{
				int width = mFont.tahoma_7b_white.getWidth(c.cName);
				Image img = Char.bluetick;
				if (img != null)
				{
					g.drawRegion(img, 0, GameCanvas.gameTick / 4 % 6 * (mGraphics.getImageHeight(img) / 6), mGraphics.getImageWidth(img), mGraphics.getImageHeight(img) / 6, 0, this.X + 64 + width, 15, mGraphics.BOTTOM | mGraphics.HCENTER);
				}
				else
				{
					Char.bluetick = GameCanvas.loadImage("/tickblue.png");
				}
			}
			if (c.cMaxStamina > 0)
			{
	...
	private void doSpeacialSkill()
	{
		...
		myVector.addElement(new Command("Mở max", AutoSpecialSkillFunctions.gI(), 4, global::Char.myCharz().infoSpeacialSkill[this.currentTabIndex][this.selected]));
	...
	private void doFireCombine()
	{
		if (currentTabIndex == 0)
		{
			...
			bool isDapDo = Functions.AutoFunctions.AutoUpgradeFunctions.isDapDo;
			if (isDapDo)
			{
			...
			if (currItem != null)
			{
				...
				else if (currItem.buyCoin == 0 && currItem.buyGold == 0)
				{
					if (currItem.powerRequire != 0)
					{
						...
						//myVector.addElement(new Command("Mua Nhiều", ItemHandler.gI(), 3, new ItemHandler.ItemAuto((int)this.currItem.template.id, -1, true, false)));
					}
					else
					{
	...
	{
		this.currentListLength = MenuHandler.MenuOption.Length;
		ITEM_HEIGHT = 24;
		selected = (GameCanvas.isTouch ? (-1) : 0);
		cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
		if (cmyLim < 0)
	...
	public void TFChat(string strChat, string strName)
	{
	...
	public void perform(int idAction, object p)
	{
		...
		if (flag22)
		{
			this.TFChat("Nhập Số Sao Cần Đập", "Chỉ Được Nhập Số");
		}
		if (idAction == 9999)
		{
		...
		if (idAction == 3144)
		{
			ItemHandler.AddItemstoList(this.currItem);
		}
		if (idAction == 170391)
		{
			Rms.clearAll();
		...
		if (idAction == 6000)
		{
			...
			bool isDapDo2 = AutoUpgradeFunctions.isDapDo;
			if (isDapDo2)
			{
				AutoUpgradeFunctions.doDeDap = item2;
			}
		}
		if (idAction == 7000)
		{
	...
	{
	//	AutoSpecialSkillFunctions.gI().onChatFromMe(text, to);
		...
		if (flag29)
		{
			AutoUpgradeFunctions.soSaoCanDap = int.Parse(text);
			GameScr.info1.addInfo(string.Concat(new object[]
			{
				"Đập ",
				AutoUpgradeFunctions.doDeDap.template.name,
				" Đến ",
				AutoUpgradeFunctions.soSaoCanDap,
				" Sao"
			}), 0);
